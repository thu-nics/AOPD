...
# Pin the complete evidence chain as well as the pools.
# Custom datasets remain supported through explicit runtime data paths.
RELEASE_FILE_SHA256 = {
...
def bundle(research_runs, output):
    if output.exists():
    ...
    for path in (deterministic, config, health):
        ...
        _json(path, value)
    _json(output / "bundle.json", {"protocol": "aopd-fixed-pools-v2", "original_sha256": original_hashes, "files": {str(relative): sha256(output / relative) for _, relative in inputs}})
    return verify(output)


def verify_release_files(output, manifest):
    """Require exact released membership and bytes, even if hashes are rewritten."""
    if manifest.get("protocol") != "aopd-fixed-pools-v2":
    ...
    if not isinstance(files, dict) or files.keys() != RELEASE_FILE_SHA256.keys():
        raise ValueError("fixed bundle file set must include every released pool and audit")
...
def main():
    ...
    print(json.dumps(bundle(args.research_runs, args.output) if args.command == "bundle" else verify(args.output)))
