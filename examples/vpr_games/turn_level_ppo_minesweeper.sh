#!/bin/bash
# ============================================================================
# Minesweeper —— Turn-Level PPO TD-GAE + outcome（结果）奖励 训练脚本
#
#   * algorithm.adv_estimator=turn_level_ppo → turn-level TD-GAE with critic values.
...
ROLLOUT_N="${ROLLOUT_N:-8}"            # vanilla 独立轨迹数；每 step 轨迹数 = TRAIN_BATCH x ROLLOUT_N
...
CRITIC_MICRO="${CRITIC_MICRO:-2}"         # critic 训练 micro-batch
...
echo "=== Minesweeper | Turn-Level PPO TD-GAE + outcome reward (32x8) ==="
...
echo "Reward: outcome only | critic: turn-level TD-GAE | oracle_process: $ORACLE_REWARD/$ORACLE_FLAG_REWARD/$ORACLE_GUESS_REWARD | non_oracle_process: $NON_ORACLE_PENALTY/$NON_ORACLE_FLAG_PENALTY | invalid: $INVALID_PENALTY | mines: $MINES"
