#!/bin/bash
# ============================================================================
# Sudoku —— 标准 GRPO + outcome（胜负）奖励 训练脚本
#
# 标准 GRPO（algorithm.adv_estimator=grpo，verl 原生 compute_grpo_outcome_advantage，
# 组内归一化）+ 纯结果奖励（env.sudoku.reward_mode=outcome）：
#   合法非终止步=0；解出整盘=+1；错填/非法动作终止=-1；超时/步数耗尽=0。
# reward_mode 默认 oracle（VPR 稠密逐步奖励）；本脚本覆盖为 outcome。
#
...
LOG_FILE="$LOG_DIR/sudoku_grpo_outcome_${TS}.log"
...
if [ ! -f "$DATA_DIR/train.parquet" ]; then
    "$PYTHON" "$SCRIPT_DIR/prepare_data.py" --env-name vpr_sudoku \
