...
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/runs/awm_data_processing/03_static_feasibility_judge}"
...
RUNTIME_CONCURRENCY="${RUNTIME_CONCURRENCY:-12}"
...
for path in \
    ...
    "$DETERMINISTIC_DIR/deterministic_manifest.json" \
    "$DETERMINISTIC_DIR/task_audit.jsonl"; do
    if [[ ! -f "$path" ]]; then
...
PY
    ...
    setsid env \
        AWM_HOST="$AWM_HOST" \
        AWM_PORT="$AWM_PORT" \
        AWM_SERVER_RUN_ID="static-feasibility-$$" \
    ...
    for _ in {1..120}; do
        if "$PYTHON" "$SCRIPT_DIR/../runtime/check_server.py" \
            ...
            --expected-run-id="static-feasibility-$$" \
    ...
    if ! "$PYTHON" "$SCRIPT_DIR/../runtime/check_server.py" \
        ...
        --expected-run-id="static-feasibility-$$" --timeout 5 >/dev/null; then
    ...
    "$PYTHON" "$SCRIPT_DIR/../runtime/check_server.py" \
        --base-url "$AWM_BASE_URL" --data-dir "$AWM_DATA_DIR" >/dev/null
fi
...
cd "$REPO_ROOT"
"$PYTHON" "$SCRIPT_DIR/build_static_feasibility_pool.py" \
    ...
    --runtime-concurrency "$RUNTIME_CONCURRENCY" \
