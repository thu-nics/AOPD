...
RUNTIME_JUDGE_PROVIDER="${RUNTIME_JUDGE_PROVIDER:-$ORACLE_PROVIDER}"
RUNTIME_JUDGE_MODEL="${RUNTIME_JUDGE_MODEL:-$ORACLE_MODEL}"
...
MATCHER_API_KEY_ENV="${MATCHER_API_KEY_ENV:-$ORACLE_API_KEY_ENV}"
MATCHER_ENABLE_THINKING="${MATCHER_ENABLE_THINKING:-null}"
MATCHER_REASONING_EFFORT="${MATCHER_REASONING_EFFORT:-null}"
MATCHER_MAX_TOKENS="${MATCHER_MAX_TOKENS:-null}"
MATCHER_MAX_CONCURRENT_REQUESTS="${MATCHER_MAX_CONCURRENT_REQUESTS:-32}"
...
TERMINAL_JUDGE_PROVIDER="${TERMINAL_JUDGE_PROVIDER:-$ORACLE_PROVIDER}"
TERMINAL_JUDGE_MODEL="${TERMINAL_JUDGE_MODEL:-$ORACLE_MODEL}"
TERMINAL_JUDGE_API_BASE="${TERMINAL_JUDGE_API_BASE:-$ORACLE_API_BASE}"
TERMINAL_JUDGE_API_KEY_ENV="${TERMINAL_JUDGE_API_KEY_ENV:-$ORACLE_API_KEY_ENV}"
...
PY
    ...
    setsid env \
        ...
        AWM_TERMINAL_JUDGE_PROVIDER="$TERMINAL_JUDGE_PROVIDER" \
    ...
    for ((attempt = 1; attempt <= AWM_SERVER_START_TIMEOUT; attempt++)); do
        ...
        if AWM_SERVER_PROTOCOL_JSON="$("$PYTHON" "$SCRIPT_DIR/../runtime/check_server.py" \
            ...
            --expected-terminal-provider "$TERMINAL_JUDGE_PROVIDER" \
...
if [[ "$VARIANT" == "agentic_opd" ]]; then
    ...
    echo "Teacher=$ORACLE_PROVIDER/$ORACLE_MODEL matcher=$MATCHER_PROVIDER/$MATCHER_MODEL runtime_judge=$RUNTIME_JUDGE_PROVIDER/$RUNTIME_JUDGE_MODEL terminal_judge=$TERMINAL_JUDGE_PROVIDER/$TERMINAL_JUDGE_MODEL"
fi
"$PYTHON" -m verl.trainer.main_ppo \
    ...
    env.awm.terminal_judge.provider="$TERMINAL_JUDGE_PROVIDER" \
    ...
    env.awm.oracle.matcher_enable_thinking="$MATCHER_ENABLE_THINKING" \
    env.awm.oracle.matcher_reasoning_effort="$MATCHER_REASONING_EFFORT" \
    env.awm.oracle.matcher_max_tokens="$MATCHER_MAX_TOKENS" \
    env.awm.oracle.matcher_max_concurrent_requests="$MATCHER_MAX_CONCURRENT_REQUESTS" \
