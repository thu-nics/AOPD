...
def test_duplicate_votes_are_not_collapsed_and_identity_is_unchanged():
    ...
    calls = []
    client = DeepSeekAWMOracleClient(request_fn=lambda p: calls.append(p) or response(True))
...
def test_structural_and_literal_differences_never_reach_llm(candidate, chat, schema):
...
def test_nested_nullable_ref_text_and_strict_arrays():
    ...
    assert build_tool_match_plan([teacher], [candidate], tools, CHAT)["pairs"] == []
...
def test_object_enum_and_boolean_numeric_structural_difference_stay_exact():
    tools = copy.deepcopy(TOOLS)
    tools[0]["inputSchema"]["enum"] = [action().arguments, action("Paid.").arguments]
    assert not build_tool_match_plan([action()], [action("Paid.")], tools, CHAT)["pairs"]
    tools[0]["inputSchema"].pop("enum")
    tools[0]["inputSchema"]["properties"]["id"] = {"type": ["boolean", "integer"]}
    assert not build_tool_match_plan([action()], [action("Paid.", id=True)], tools, CHAT)["pairs"]
